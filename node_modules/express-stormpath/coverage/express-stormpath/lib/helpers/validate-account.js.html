...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (customFieldNames.indexOf(field.name) &gt;= 0 &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >isSet(formData.customData[field.name]))</span> {
  ...
  getRequiredRegistrationFields(stormpathConfig, function (requiredFields) {
    async.each(requiredFields, function (field, cb) {
      if (!isFieldIncluded(field)) {
        errors.push(new Error((field.label || <span class="branch-1 cbranch-no" title="branch not covered" >field.name)</span> + ' required.'));
      }
&nbsp;
      cb();
    }, function () {
...
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (confirmPasswordField &amp;&amp; confirmPasswordField.enabled) {
<span class="cstat-no" title="statement not covered" >        if (formData.password !== formData.confirmPassword) {</span>
<span class="cstat-no" title="statement not covered" >          errors.push(new Error('Passwords do not match.'));</span>
        }
      }
&nbsp;
      var coreFields = [
